...
<head>
    ...
    <style>
        ...
        .navbar-macha {
            ...
            box-shadow: 0 2px 20px rgba(45, 90, 39, .08);
            padding: 13px 0;
        }
        ...
        .step.active .step-circle {
            ...
            color: #fff;
            border-color: var(--gd);
            box-shadow: 0 0 0 6px rgba(27, 77, 62, .15), 0 6px 16px rgba(27, 77, 62, .4);
        ...
        .step.done+.step-line .step-line-fill {
            width: 100%;
        }
        ...
        .pay-option.selected {
            ...
            box-shadow: 0 4px 20px rgba(64, 145, 108, .12);
        }
        ...
        /* SUMMARY SIDEBAR */
        .summary-card {
            background: linear-gradient(165deg, var(--gd), #0d2a1f);
            color: #fff;
            border-radius: 28px;
            padding: 32px;
            position: sticky;
            top: 90px;
            box-shadow: 0 15px 45px rgba(27, 77, 62, .25);
            overflow: hidden;
        }
        ...
        .note-check-label:hover {
            border-color: var(--gl);
            background: #f9fbf9;
        }

        .note-check-label input {
            display: none;
        }

        .note-check-label.active {
            background: var(--gm);
            color: #fff;
            border-color: var(--gm);
...
<body>
    ...
    <div class="steps-bar">
        <div class="steps">
            ...
            <div class="step-line">
                <div class="step-line-fill" id="line1"></div>
            </div>
            ...
            <div class="step-line">
                <div class="step-line-fill" id="line2"></div>
            </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-7">
                <form id="checkoutForm" action="<?= base_url('shop/process_checkout') ?>" method="post">
                    ...
                    <div class="form-card mt-4">
                        ...
                        <div id="paymentInfoBox" class="mt-4 p-4 rounded-4 d-none" style="background:#f0faf4; border: 2px dashed var(--gl);">
                            <div class="d-flex gap-3">
                                ...
                                <div>
                                    <h6 class="fw-bold text-dark mb-1" id="paymentInfoTitle">Instruksi Pembayaran</h6>
                                    <p class="text-secondary mb-0" id="paymentInfoText"></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-card">
                        <h5><i class="fa-solid fa-truck-fast me-2"></i>Informasi & Alamat</h5>
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label">Nama Pemesan</label>
                                <input type="text" name="customer_name" class="form-control"
                                    value="<?= htmlspecialchars($user['full_name'] ?? $this->session->userdata('full_name')) ?>"
                                    required>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div id="addressSection" class="d-none">
                                <div class="col-12 mt-3">
                                    ...
                                    <textarea name="address" id="addressInput" class="form-control" rows="3"
                                        placeholder="Jl. Contoh No. 123, blok A..."><?= htmlspecialchars($user['address'] ?? '') ?></textarea>
                                </div>
                                <div class="col-12 mt-3">
                                    <label class="form-label d-flex justify-content-between">
                                        <span>Titik Lokasi Maps (Bantu Kurir)</span>
                                        <button type="button" class="btn btn-sm btn-outline-success border-0 p-0"
                                            onclick="getLocation()" style="font-size: .8rem; font-weight: 700;">
                                            <i class="fa-solid fa-location-dot me-1"></i>Bagikan Lokasi
                                        </button>
                                    </label>
                                    <div class="input-group">
                                        <span class="input-group-text bg-light border-2 border-end-0"
                                            style="border-radius: 14px 0 0 14px;"><i
                                                class="fa-solid fa-link text-muted"></i></span>
                                        <input type="text" name="google_maps_link" id="google_maps_link"
                                            class="form-control border-start-0" style="border-radius: 0 14px 14px 0;"
    ...
    <script>
        ...
        function selectType(el, type) {
            ...
            document.querySelectorAll('[id^="type_"]').forEach(o => o.classList.remove('selected'));
            ...
            if (type === 'delivery') {
                ...
                setTimeout(() => { addressSection.style.opacity = '1'; addressSection.style.transform = 'translateY(0)'; }, 50);
            } else {
                addressSection.style.opacity = '0';
                addressSection.style.transform = 'translateY(-10px)';
                setTimeout(() => addressSection.classList.add('d-none'), 300);
            }
        }

        // PAYMENT METHOD SELECTION (DYNAMIC)
        function selectPay(el, method, desc) {
            // Remove selected class from all payment options
            document.querySelectorAll('[id^="pay_"]').forEach(o => o.classList.remove('selected'));
            // Add selected to current
            el.classList.add('selected');
            // Check the radio
            el.querySelector('input[type="radio"]').checked = true;
