...
class Requests extends CI_Controller {
    ...
    public function update_status($id, $status) {
        $supplier_id = $this->session->userdata('supplier_id');
        $valid_statuses = ['approved', 'processing'];
        
        if (in_array($status, $valid_statuses)) {
            $this->Supplier_model->update_request_status($id, $status, $supplier_id);
    ...
    public function reject($id) {
        ...
        $this->Supplier_model->reject_request($id, $supplier_id, $reason);
    ...
    public function dt_json() {
        ...
        foreach ($requests as $req) {
            ...
            if ($req['status'] === 'rejected' && !empty($req['rejection_reason'])) {
                $badge .= '<div style="font-size:0.75rem;color:#ef4444;margin-top:4px;max-width:150px;white-space:normal;line-height:1.2;">Alasan: '.htmlspecialchars($req['rejection_reason']).'</div>';
            }
            if ($req['status'] == 'pending') {
                $actions = '<div style="display:flex;gap:6px;">'
                    .'<a href="'.base_url('supplier/requests/update_status/'.$req['id'].'/approved').'" onclick="return confirm(\'Approve request ini?\')" style="background:#dcfce7;color:#166534;padding:4px 10px;border-radius:8px;font-size:0.75rem;font-weight:700;text-decoration:none;">Approve</a>'
                    .'<a href="javascript:void(0)" onclick="rejectRequest('.$req['id'].')" style="background:#fee2e2;color:#991b1b;padding:4px 10px;border-radius:8px;font-size:0.75rem;font-weight:700;text-decoration:none;">Reject</a>'
                    .'</div>';
            ...
            $rows[] = [
                ...
                $req['quantity'],
                !empty($req['notes']) ? htmlspecialchars($req['notes']) : '-',
